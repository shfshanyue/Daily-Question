...
# 此时，npm 可以利用缓存，如果package-lock.json 内容没有变化，则不会重新依赖安装
RUN npm i
...
...
 ...
 => CACHED [3/6] ADD package.json package-lock.json /code/                      0.0s
 => CACHED [4/6] RUN npm i                                                      0.0s
 => CACHED [5/6] ADD . /code                                                    0.0s
